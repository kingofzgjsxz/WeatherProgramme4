...
import com.truezir.android.weatherprogramme.AirQuality;
...
public class WeatherForecastByDayFetch {
    //测试信息:TAG
    ...
    //API信息:网址
    private static final String uri="https://api.heweather.net/s6/weather/forecast?";
    ...
    //解析Gson数据
    /**注意更改此处的类的类型*/
    private static WeatherForecastByDay handleAirQuality(String response){
        try{
...
//            JSONArray jsonArray=new JSONArray(response);
            ...
            return new Gson().fromJson(weatherContent, WeatherForecastByDay.class);
    ...
    public WeatherForecastByDay getWeatherForecastByDay(){

        mFetchItems=fetchItems();//接收网络所传的字符串;

        /**注意更改此处的类的类型*/

        WeatherForecastByDay mWeatherForecastByDay=handleAirQuality(mFetchItems);

        return mWeatherForecastByDay;
    }
}
